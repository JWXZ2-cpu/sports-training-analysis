...
**脉（MAI）** 是一套面向专业运动队的 **AI 驱动多角色训练分析平台**，覆盖运动员、主教练、助教、队医、管理人员五大角色。系统以"语音/文字输入 → AI 分析 → 角色化分发"为核心数据流，整合丹尼尔斯跑步训练法、邦帕周期化训练理论和运动心理学三大理论框架，为教练团队提供科学的训练决策支持。

系统支持中/英/意三语切换，采用移动端优先的暗色主题设计（430px 响应式），适配外教和国际运动员的使用场景。支持多团队邀请码隔离，不同队伍数据完全独立。
...
│   │   └── it.js                 # 意大利语语言包
│   ├── services/
│   │   ├── api.js                # HTTP 客户端（自动注入 Token、安全 JSON 解析）
│   │   └── index.js              # 13 个 service barrel export（含 teamService）
│   ├── utils/
│   │   ├── mimoAsr.js            # 浏览器端录音 + ASR（MediaRecorder + AudioContext 双方案）
...
│   │   ├── ManagerProfile.jsx
│   │   ├── TeamManagement.jsx    # 团队管理（主教练）
│   │   ├── TeamOnboarding.jsx    # 团队创建引导（主教练首次登录）
│   │   ├── VoiceInput.jsx        # 通用语音输入
│   │   ├── DataConfirm.jsx       # 数据确认
│   │   └── ResultPage.jsx        # 结果展示
│   └── components/               # 20 个共享组件
│       ├── AthleteGuard.jsx      # 运动员路由守卫（首次登录体测引导）
│       ├── CoachGuard.jsx        # 主教练路由守卫（首次登录创建团队）
│       ├── TeamJoinCard.jsx      # 加入团队卡片（无团队用户）
│       ├── CoachNav.jsx          # 教练底栏导航
...
| 表名 | 说明 | 关键字段 |
|---|---|---|
| `teams` | 团队 | name, invite_code, owner_id |
| `users` | 用户账号 | username, password_hash, role, display_name, **team_id** |
| `athlete_profiles` | 运动员档案 | sport, vdot, max_hr, resting_hr, threshold_pace, **physiology** |
...
JWT payload 包含 `{ id, username, role, display_name, team_id }`，团队隔离信息随 token 传递。

```
用户登录 → 服务端验证密码 → 签发 JWT (24h, 含 team_id) → 客户端存入 localStorage
     ↓
...
|---|---|---|
| POST | `/` | 创建团队（主教练专用） |
| GET | `/my` | 获取我的团队信息（含成员列表） |
| POST | `/invite-code/regenerate` | 重新生成邀请码 |
| POST | `/verify-code` | 验证邀请码（注册时调用） |
| GET | `/members` | 获取团队成员列表 |

### 运动员体测 `/api/athlete/onboarding`

| Method | Path | 说明 |
|---|---|---|
| POST | `/vdot` | PB 成绩 → VDOT → 5 区训练区间 |
| POST | `/conconi` | Conconi 测试 → 训练区间 |
| POST | `/save` | 保存体测数据到 athlete_profiles |
| GET | `/status` | 检查体测状态（含 6 周过期检测） |
...
所有数据查询自动加 `team_id` 过滤，通过两个 helper 函数实现：

```js
// server/middleware/auth.js
export function getTeamUserIds(teamId)    // 获取团队内所有用户 ID
export function isUserInTeam(userId, teamId) // 检查用户是否在指定团队
```
...
| 数据类型 | 隔离方式 |
|---|---|
| 用户列表 | `db.users.findAll({ team_id })` 直接过滤 |
| 训练记录/计划/报告/通知 | 通过 `getTeamUserIds()` 获取团队成员 ID，再用 `$in` 过滤 |
| 伤病/治疗记录 | 同上 |
| 绑定关系 | 验证所有引用的用户 ID 是否在同一团队 |
| 单条记录访问 | `isUserInTeam()` 检查后返回 403 |

### 注册流程

```
用户注册 → 输入邀请码（选填）→ 验证码有效性 → 关联到团队
主教练注册 → 邀请码留空 → 登录后创建团队 → 获得邀请码
```
